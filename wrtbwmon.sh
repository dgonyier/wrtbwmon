...
"update" )
	...
	do
		...
		if [ ${IN} -gt 0 -o ${OUT} -gt 0 ];  then
			echo "DEBUG : New traffic for ${MAC} (${IP}) since last update : ${IN}k:${OUT}k"
			LINE=$(grep "${MAC}" "${2}")
			if [ -z "${LINE}" ]; then
			...
			mv /tmp/db_$$.tmp ${2}
			echo "${IP}",${MAC},${PEAKUSAGE_IN},${PEAKUSAGE_OUT},${OFFPEAKUSAGE_IN},${OFFPEAKUSAGE_OUT},$(date "+%d-%m-%Y %H:%M") >> ${2}
		fi
	done
...
"publish" )
	...
	cat ${2} | while IFS=, read IP MAC PEAKUSAGE_IN PEAKUSAGE_OUT OFFPEAKUSAGE_IN OFFPEAKUSAGE_OUT LASTSEEN
	do
		echo ${IP},${PEAKUSAGE_IN},${PEAKUSAGE_OUT},${OFFPEAKUSAGE_IN},${OFFPEAKUSAGE_OUT},${MAC},${LASTSEEN} >> /tmp/sorted_$$.tmp
	done
	unlock
        ...
        echo "<table border=\"1\"><tr bgcolor=silver><th>User</th><th>IP</th><th>Peak download</th><th>Peak upload</th><th>Offpeak download</th><th>Offpeak upload</th><th>Last seen</th></tr>" >> ${3}
        ...
        echo "document.write(values[i][1]);document.write(\"</td><td>\");" >> ${3}
        ...
        echo "document.write(getSize(values[i][5]));document.write(\"</td><td>\");" >> ${3}
        echo "document.write(values[i][6]);document.write(\"</td></tr>\");" >> ${3}
        echo "}</script></table>" >> ${3}
